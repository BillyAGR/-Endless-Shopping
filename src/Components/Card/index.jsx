...
const Card = ({ data }) => {
    ...
    const { name, image } = category ? category : ''

    const showProduct = () => {
        context.openProductDetail()
        context.setProductToShow(data)
        context.closeCheckoutSideMenu()
        context.setDisplayMode('hidden')
    }
    ...
    const imageUrl = images ? changeUrl(images[0]) : ''
    ...
    return (
        <div className='bg-white cursor-pointer w-56 h-60 rounded-lg'
            ...
            <figure className='relative mb-2 w-full h-4/5'>
                ...
                <div className={`absolute top-0 right-0 flex justify-center items-center ${isInCart ? 'bg-black' : 'bg-white hover:bg-gray-300'} w-6 h-6 rounded-full m-2 p-1`}
